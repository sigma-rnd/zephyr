...
static inline int lps22hh_set_odr_raw(const struct device *dev, uint8_t odr)
{
	...
	if (odr >= 0x1 && odr <= 0x5) {
		...
		LOG_WRN("Low noise not supported with ODR >= 100 Hz (Current ODR=0x%02x)", odr);
	}
#endif
					 ...
					 int32_t raw_val)
{
	...
	val->val1 = (press_tmp >> 12) / 10;
	val->val2 = (press_tmp >> 12) % 10 * 100000 +
		(((int32_t)((press_tmp) & 0x0FFF) * 100000L) >> 12);
}
